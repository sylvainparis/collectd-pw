...
Example :
 ...
 <Plugin "nfs">
   ...
   <mountpoint "/mynfsmountpoints/data1">
     min_age 0
     perop_statistics "all"
     show true
   </mountpoint>
   <mountpoint "/mynfsmountpoints/data2">
     min_age 0
     perop_statistics "READ,WRITE,GETATTR"
     show true
   </mountpoint>
 </Plugin>
...
=item B<perop_statistics> B<string>

=over

=item *

If not empty, this is a CSV list of per-op statistics to collect from /proc/self/mountpoint.

=item *

If empty or missing, no per-op statistics will be collected.

=item *

If set to "all", all per-op statistics will be collected.
